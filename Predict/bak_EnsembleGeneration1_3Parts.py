...
oof_train_LR = np.zeros((ntrain))
oof_test_LR = np.zeros((ntest))
oof_test_skf_LR = np.empty((5, ntest))
...
for i, (train_index, test_index) in enumerate(kf.split(x_train)):
    ...
    clf = LogisticRegression(
        penalty="l2", C=0.3, max_iter=400, tol=1e-4, solver="lbfgs"
    )
    clf.fit(x_train[train_index], y_train[train_index])
    oof_train_LR[test_index] = clf.predict_proba(kf_x_test)[:, 1]
    oof_test_skf_LR[i, :] = clf.predict_proba(x_test)[:, 1]
    (
        AUC_value,
        ACC_value,
        Recall_value,
...
np.savetxt(
    "./Data/EnsembleGeneration/oof_test_LR.csv",
    oof_test_LR,
    encoding="utf-8-sig",
    delimiter=",",
    fmt="%.5f",
...
np.savetxt(
    "./Data/EnsembleGeneration/oof_train_LR.csv",
    oof_train_XGB,
    encoding="utf-8-sig",
    delimiter=",",
    fmt="%.5f",
