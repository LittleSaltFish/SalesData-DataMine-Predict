...
f1 = str(f1_score(y_test, y_pred, average="binary"))[:7]

# 保存模型
bst.save_model(f"./Predict/Model/test-{f1}.model")
# 导出模型到文件
bst.dump_model(f"./Predict/Model/dump-{f1}.raw.txt")
# 导出模型和特征映射
...
(
    ...
    ConfusionMatrix,
    Report,
) = MakeReport("XGBoost_f1", y_test, y_pred, argument, ypred)
# DrawTree(bst,f1)
